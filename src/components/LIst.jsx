...
const List = ({ id, task, isDone, setDone, deleteTask, updateTask }) => {
  ...
  return (
    <div className="list">
      <div
        className={`group animate__animated animate__fadeInLeft border mb-4 overflow-hidden border-neutral-700 ${
          edit ? "py-3 px-4" : "py-4 px-5"
        } flex justify-between items-center`}
      >
        {edit ? (
          <input
            className=" border border-neutral-500 w-[270px] py-1 px-2"
            type="text"
            id={`list-update-${id}`}
            value={updateInput}
          ...
          <div className="content flex items-center gap-3">
            ...
            <label
              htmlFor={`list-${id}`}
              className={`list-text select-none ${isDone && "line-through"}`}
            >
              {task}
            </label>
          </div>
